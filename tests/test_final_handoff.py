...
def test_final_handoff_is_the_first_repository_entrypoint():
    ...
    assert readme.index("FINAL_HANDOFF_CN.md") < readme.index("START_HERE_CN.md")
    assert agents.index("FINAL_HANDOFF_CN.md") < agents.index("START_HERE_CN.md")
...
def test_historical_active_plan_is_explicitly_closed():
    ...
    assert "最终状态覆盖" in start[:700]
    assert "最终交接覆盖" in capsule[:700]
