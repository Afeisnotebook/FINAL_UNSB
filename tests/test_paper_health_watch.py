...
def test_watch_is_healthy_without_copying_losses(tmp_path: Path) -> None:
    ...
    row = evaluate_watch(
        parse_watch(f"plain|123|{path}|200|0"),
        now=1000.0,
        watch_started=800.0,
...
def test_terminal_state_allows_supervisor_to_exit(tmp_path: Path) -> None:
    ...
    row = evaluate_watch(
        parse_watch(f"plain|123|{path}|10|0"),
        now=1000.0,
        watch_started=0.0,
        alive=lambda _pid: False,
    )
...
def test_non_epoch_complete_successor_state_is_terminal(tmp_path: Path) -> None:
    path = tmp_path / "state.json"
    _write(
        path,
    ...
    row = evaluate_watch(
        parse_watch(f"pathology|123|{path}|10|0"),
        now=1000.0,
        watch_started=0.0,
        alive=lambda _pid: False,
    )
...
def test_contract_rejects_relative_disk_path(tmp_path: Path) -> None:
    args = Namespace(
        ...
        host_label="host",
        watch=[f"plain|123|{tmp_path / 'state.json'}|3600|0"],
        disk_path=Path("relative"),
        estimated_remaining_write_gib=1.0,
        minimum_headroom_gib=1.0,
        user_capacity_override=False,
        poll_seconds=60,
        timeout_hours=480.0,
        once=False,
    )
