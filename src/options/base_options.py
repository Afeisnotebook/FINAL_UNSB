...
class BaseOptions():
    ...
    def __init__(self, cmd_line=None):
        ...
        if cmd_line is not None:
            # Production orchestration passes an argv-style list so paths and
            # values containing spaces are never reparsed incorrectly.
            self.cmd_line = (
                list(cmd_line)
                if isinstance(cmd_line, (list, tuple))
                else cmd_line.split()
            )
