...
截至`2026-08-31 00:20 +08:00`，AM-TNC与MCRB均已完成首个晚期点e150，MCRB已继续
e155。MCRB的真实e100描述性delta为`+2.158 dB`，但已验证它不会触发早停、4090复赛、排名或冻结，科学硬门
...
不授权退出、改算法或机制判死。
AM-TNC同样从e100的`+1.660 dB`、6/6域正转为e125的`-0.333 dB`、2/6域正；不同之处是
候选自身仍绝对提升`1.452 dB`，只是同宿主plain提升更快（`3.444 dB`）。因此当前证据指向
“相对优势未被保持”，而不是候选自身发生坍塌；它仍按冻结公式继续到e200。
MCRB在首个晚期点e150重新达到`+0.894 dB`且6/6域正；e125→e150候选自身提升
`1.074 dB`，plain同期回撤`0.590 dB`。该点同时包含候选进步与plain波动，只有
e175/e200和完整terminal receipt才能决定是否触发4090同宿主复赛。
AM-TNC e150为`+0.537 dB`、5/6域正，但LPIPS差`0.00595`；e125→e150候选自身仅提升
`0.259 dB`，plain回撤`0.611 dB`。所以其相对转正主要来自plain下行，当前仍未通过
完整护栏，同样必须继续到e175/e200。
