...
5090A 上的 v2 首进程 PID `436173` 产生健康状态后被有意停止，并使用完全相同的输出目录
与命令重启为 PID `436478`；冻结合同未冲突，第二个 30 秒探针继续报告真实 I/O 增长和
零告警。随后才退休 v1 PID `435350`。ST-CGR supervisor/trainer 始终保持
`336345/431032`，证明监控替换与训练隔离。
