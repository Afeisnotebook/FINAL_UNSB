"""Durably run G3-03 after strict 4090 parents and the G3-02 ledger freeze."""
...
from operations.local_route1_freeze_residual_euclidean_synthesis import (
    CANDIDATE_ID,
    PCRSMG_PROPOSAL_ID,
    RFMCRB_ID,
    materialize,
...
SCHEMA = "final-unsb-route1-residual-euclidean-synthesis-4090-successor-contract-v1"
...
SOURCE_RELATIVES = (
    "operations/local_route1_residual_euclidean_synthesis_4090_successor.py",
    "operations/local_route1_freeze_residual_euclidean_synthesis.py",
...
def _git_identity(repo: Path) -> dict[str, str]:
    ...
    if support.run_text(["git", "status", "--porcelain"], cwd=repo):
        raise RuntimeError("Euclidean synthesis worktree is dirty")
    return {
        "path": str(repo),
...
def default_contract(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for path, label in (
        ...
        if not path.is_file():
            raise RuntimeError(f"Euclidean synthesis {label} is missing: {path}")
    receipt = _validate_receipt(sampling)
    if receipt.get("candidate_id") != PCRSMG_PROPOSAL_ID:
        raise RuntimeError("4090 Euclidean synthesis requires PC-RSMG proposal parent")
    return {
        "schema": SCHEMA,
        "created": support.now(),
        ...
        "required_barrier_parent_id": RFMCRB_ID,
        "required_prior_ledger_boundary_candidate_id": ADAM_SYNTHESIS_ID,
        "batch_size": 1,
...
def validate_contract(contract: dict[str, Any]) -> None:
    if contract.get("schema") != SCHEMA:
        raise RuntimeError("Euclidean synthesis successor contract schema mismatch")
    repo = Path(str(contract.get("repo", {}).get("path", ""))).resolve()
    if support.run_text(["git", "rev-parse", "HEAD"], cwd=repo) != contract[
        "repo"
    ].get("git_commit"):
        raise RuntimeError("Euclidean synthesis worktree moved")
    if support.run_text(["git", "status", "--porcelain"], cwd=repo):
        raise RuntimeError("Euclidean synthesis worktree became dirty")
    for relative, expected in contract.get("source_sha256", {}).items():
        if support.file_sha256(repo / relative) != expected:
            raise RuntimeError(f"Euclidean synthesis source changed: {relative}")
    for key in ("sampling_receipt", "manifest", "baseline_environment_record"):
        if support.file_sha256(Path(contract[key])) != contract.get(f"{key}_sha256"):
            raise RuntimeError(f"Euclidean synthesis authority changed: {key}")
    fixed = {
        "candidate_id": CANDIDATE_ID,
        "required_sampling_parent_id": PCRSMG_PROPOSAL_ID,
        "required_barrier_parent_id": RFMCRB_ID,
    ...
    for key, expected in fixed.items():
        if contract.get(key) != expected:
            raise RuntimeError(f"Euclidean synthesis contract changed: {key}")
    if int(contract.get("poll_seconds", 0)) < 15:
        raise RuntimeError("Euclidean synthesis polling is too frequent")
    if int(contract.get("timeout_seconds", 0)) < 43200:
        raise RuntimeError("Euclidean synthesis timeout is too short")


class ResidualEuclideanSynthesis4090Successor:
    def __init__(self, contract_path: Path):
        ...
        self.state_path = (
            self.operations / "RESIDUAL_EUCLIDEAN_SYNTHESIS_4090_SUCCESSOR_STATE.json"
        )
    ...
    def state(self, status: str, **fields: Any) -> None:
        support.atomic_json(self.state_path, {
            "schema": (
                "final-unsb-route1-residual-euclidean-synthesis-4090-"
                "successor-state-v1"
            ),
            "updated": support.now(),
            "status": status,
    ...
    def wait_prior_ledger_boundary(self) -> None:
        freeze_path = self.operations / "RESIDUAL_SYNTHESIS_FREEZE.json"
        result_path = self.operations / "RESIDUAL_SYNTHESIS_4090_RESULT.json"
        while not (freeze_path.is_file() or result_path.is_file()):
            if self._timed_out():
                raise TimeoutError("timed out waiting for G3-02 ledger boundary")
            self.state("WAITING_FOR_G3_02_SHARED_LEDGER_FREEZE")
            time.sleep(int(self.contract["poll_seconds"]))
        if freeze_path.is_file():
            value = _read_json(freeze_path)
            if value.get("paired_controller_access") is not False or value.get(
                "confirmation20_opened"
            ) is not False:
                raise RuntimeError("G3-02 freeze violates route-1 scope")
        else:
            value = _read_json(result_path)
            if value.get("candidate_id") not in (None, ADAM_SYNTHESIS_ID):
                raise RuntimeError("unexpected G3-02 ledger-boundary result")
    ...
    def _write_inapplicable(self, status: str, **fields: Any) -> int:
        path = self.operations / "RESIDUAL_EUCLIDEAN_SYNTHESIS_4090_RESULT.json"
        result = {
            "schema": "final-unsb-route1-residual-euclidean-synthesis-4090-result-v1",
            "status": status,
            "candidate_id": None,
            "old_fixed_margin_operator_run": False,
            "paired_controller_access": False,
            "confirmation20_opened": False,
            **fields,
        }
        support.atomic_json(path, result)
        self.state(status, result_sha256=support.file_sha256(path))
        return 0
    ...
    def run(self) -> int:
        ...
        if RFMCRB_ID not in rows:
            return self._write_inapplicable(
                "SYNTHESIS_INAPPLICABLE_RFMCRB_NOT_REPLAYED"
            )
        self.wait_prior_ledger_boundary()
        barrier_row = rows[RFMCRB_ID]
        ...
        if (
            ...
            raise RuntimeError("RF-MCRB 4090 parent receipt changed")
        ...
        if freeze["status"] == "SYNTHESIS_INAPPLICABLE":
            return self._write_inapplicable(
                "SYNTHESIS_INAPPLICABLE_ONE_OR_MORE_PARENTS_NOT_STRICT",
                freeze_sha256=support.file_sha256(
                    self.operations / "RESIDUAL_EUCLIDEAN_SYNTHESIS_FREEZE.json"
                ),
            )
        if freeze["status"] != "SYNTHESIS_FROZEN_FOR_COMPATIBILITY_GATE":
            raise RuntimeError("unexpected Euclidean synthesis freeze outcome")
        ...
        self._run_checked([
            ...
            "--gpu", "0",
        ], label="Euclidean synthesis compatibility gate", timeout=21600)
        executor = (
            self.operations / f"CANDIDATE_EXECUTOR_CONTRACT_{CANDIDATE_ID}_4090.json"
        )
        if not executor.is_file():
            ...
            ], label="Euclidean synthesis executor freeze")
        self.state("RESIDUAL_EUCLIDEAN_SYNTHESIS_E200_RUNNING")
        self._run_checked([
            self.contract["python"],
            str(self.repo / "operations/local_route1_candidate_executor.py"),
            "--contract", str(executor),
        ], label="Euclidean synthesis e200", timeout=int(self.contract["timeout_seconds"]))
        receipt_path = (
        ...
        ], label="Euclidean synthesis receipt", timeout=1800)
        receipt = _validate_receipt(receipt_path)
        result = {
            "schema": "final-unsb-route1-residual-euclidean-synthesis-4090-result-v1",
            "status": "RESIDUAL_EUCLIDEAN_SYNTHESIS_4090_COMPLETE_E200",
            "candidate_id": CANDIDATE_ID,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        with support.executor_lock(
            run_root / "operations" /
            "RESIDUAL_EUCLIDEAN_SYNTHESIS_4090_SUCCESSOR.lock"
        ):
            return ResidualEuclideanSynthesis4090Successor(args.contract).run()
    except Exception as error:
        support.atomic_json(
            run_root / "operations" /
            "RESIDUAL_EUCLIDEAN_SYNTHESIS_4090_SUCCESSOR_FATAL.json",
            {
                "schema": (
                    "final-unsb-route1-residual-euclidean-synthesis-4090-"
                    "successor-fatal-v1"
