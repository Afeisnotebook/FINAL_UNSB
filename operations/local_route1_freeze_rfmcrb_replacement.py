"""Register and freeze the residual-feasible Euclidean MCRB repair."""
...
PARENT_ID = "G1-03-STATE-FEEDBACK-MISSING"
CANDIDATE_ID = "F2-02-RESIDUAL-FEASIBLE-EUCLIDEAN-COVARIANCE-BARRIER"
INCIDENT = (
...
SPEC = {
    ...
    "sources": [
        ...
        "src/models/route1_rfmcrb_model.py",
...
def _copy_exact(source: Path, destination: Path) -> None:
    ...
    if destination.is_file():
        if destination.read_bytes() != source.read_bytes():
            raise RuntimeError(f"non-identical frozen RF-MCRB artifact: {destination}")
        return
...
def materialize(output_root: Path) -> dict:
    ...
    if implementation_path.is_file():
        ...
        if existing != implementation:
            raise RuntimeError("non-identical RF-MCRB implementation already exists")
    else:
    ...
    return {
        ...
        "status": "RFMCRB_REPLACEMENT_FROZEN_FOR_GATES",
