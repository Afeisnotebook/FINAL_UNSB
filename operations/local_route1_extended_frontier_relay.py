"""Atomically relay the portable 5090 extended frontier to the 4090 host."""
...
SCHEMA = "final-unsb-route1-extended-frontier-relay-contract-v1"
PASSWORD_ENV = {
...
def validate_contract(contract: dict[str, Any]) -> None:
    if contract.get("schema") != SCHEMA:
        raise RuntimeError("extended frontier relay contract schema mismatch")
    for role in ("source", "destination"):
        endpoint = contract.get(role)
        if not isinstance(endpoint, dict):
            raise RuntimeError(f"extended frontier relay lacks {role} endpoint")
        for key in ("host", "user", "path"):
            if not isinstance(endpoint.get(key), str) or not endpoint[key]:
                raise RuntimeError(f"extended frontier relay {role} lacks {key}")
        if not 1 <= int(endpoint.get("port", 0)) <= 65535:
            raise RuntimeError(f"extended frontier relay {role} port is invalid")
        if any("password" in str(key).lower() for key in endpoint):
            raise RuntimeError("extended frontier relay must never persist a password")
    fixed = {
        "maximum_consecutive_failures": 3,
        "complete_source_e200_only": True,
        "destination_overwrite_allowed": False,
    ...
    for key, expected in fixed.items():
        if contract.get(key) != expected:
            raise RuntimeError(f"extended frontier relay changed: {key}")
    if int(contract.get("poll_seconds", 0)) < 30:
        raise RuntimeError("extended frontier relay polling is too frequent")
    if int(contract.get("timeout_seconds", 0)) < 43200:
        raise RuntimeError("extended frontier relay timeout is too short")
    if Path(contract["local_spool"]).resolve() == Path(contract["state_path"]).resolve():
        raise RuntimeError("extended frontier relay spool and state must differ")
...
class ExtendedFrontierRelay:
    def __init__(self, contract_path: Path):
        ...
        for role, variable in PASSWORD_ENV.items():
            ...
            if not value:
                raise RuntimeError(f"missing extended frontier relay password: {variable}")
            self.passwords[role] = value

    def state(self, status: str, **fields: Any) -> None:
        support.atomic_json(self.state_path, {
            "schema": "final-unsb-route1-extended-frontier-relay-state-v1",
            "updated": support.now(),
            "status": status,
            "pid": os.getpid(),
    ...
    def source_ready(self) -> bool:
        ...
        try:
            ...
            client.close()
        validate_portable_extended_frontier(_read_json(self.spool))
        return True
            ...
            with client.open_sftp() as sftp:
                ...
                if existing is not None:
                    if existing != local_sha:
                        raise RuntimeError("extended frontier destination differs")
                    return local_sha
                sftp.put(str(self.spool), temporary)
                if _remote_sha256(sftp, temporary) != local_sha:
                    raise RuntimeError("extended frontier temporary upload changed")
                sftp.rename(temporary, endpoint["path"])
                if _remote_sha256(sftp, endpoint["path"]) != local_sha:
                    raise RuntimeError("extended frontier destination hash changed")
        finally:
            client.close()
        return local_sha
    ...
    def run(self) -> int:
        ...
        while True:
            if time.time() - self.started > int(self.contract["timeout_seconds"]):
                raise TimeoutError("extended frontier relay timed out")
            try:
                ...
                self.state("VALIDATED_PORTABLE_EXTENDED_5090_FRONTIER", source_sha256=source_sha)
                destination_sha = self.upload_exact()
                self.state(
                    "COMPLETE_EXTENDED_FRONTIER_RELAYED_TO_4090",
                ...
                failures += 1
                self.state(
                    "TRANSIENT_EXTENDED_FRONTIER_RELAY_FAILURE",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        return ExtendedFrontierRelay(args.contract).run()
    except Exception as error:
        support.atomic_json(state_path.with_name("EXTENDED_FRONTIER_RELAY_FATAL.json"), {
            "schema": "final-unsb-route1-extended-frontier-relay-fatal-v1",
            "updated": support.now(),
            "status": "FAILED",
            "error": repr(error),
