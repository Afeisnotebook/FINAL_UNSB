"""Durably retrieve the complete route-1 delivery from the 4090 authority."""
...
SCHEMA = "final-unsb-route1-complete-final-result-relay-contract-v1"
...
def validate_pointer(value: dict[str, Any]) -> dict[str, Any]:
    if (
        ...
        raise RuntimeError("complete final relay pointer is not terminal")
    fixed = {
    ...
    for key, expected in fixed.items():
        if value.get(key) != expected:
            raise RuntimeError(f"complete final relay pointer changed: {key}")
    hashes = value.get("final_file_sha256")
    if not isinstance(hashes, dict) or set(hashes) != set(PUBLISHED_FILES):
        raise RuntimeError("complete final relay file set changed")
    if int(value.get("research_frontier_unique_candidate_count", 0)) < 1:
        raise RuntimeError("complete final relay lacks a research frontier")
    return value
...
def validate_local_delivery(destination: Path) -> dict[str, Any]:
    ...
    for name, expected in pointer["final_file_sha256"].items():
        if support.file_sha256(destination / name) != expected:
            raise RuntimeError(f"relayed final file changed: {name}")
    extras = {
        EXTRA_FILES[0]: pointer["complete_4090_frontier_sha256"],
        EXTRA_FILES[1]: pointer["portable_5090_frontier_sha256"],
    }
    for name, expected in extras.items():
        if support.file_sha256(destination / name) != expected:
            raise RuntimeError(f"relayed terminal input changed: {name}")
    candidate = _read_json(destination / "CANDIDATE.json")
    research = _read_json(destination / "RESEARCH_FRONTIER.json")
    if (
        candidate.get("candidate_id") != pointer.get("selected_candidate_id")
        or candidate.get("canonical_candidate_is_action_priority_only") is not True
        or candidate.get("algorithm_discovery_collapsed_to_single_candidate") is not False
        or research.get("action_priority_candidate_id") != pointer.get(
            "selected_candidate_id"
        )
        or research.get("algorithm_discovery_collapsed_to_single_candidate") is not False
        or research.get("confirmation20_opened") is not False
    ):
        raise RuntimeError("relayed candidate/research-frontier identity changed")
    return pointer
...
def validate_contract(contract: dict[str, Any]) -> None:
    if contract.get("schema") != SCHEMA:
        raise RuntimeError("complete final relay contract schema mismatch")
    source = contract.get("source")
    if not isinstance(source, dict):
        raise RuntimeError("complete final relay lacks source")
    for key in ("host", "user", "run_root"):
        if not isinstance(source.get(key), str) or not source[key]:
            raise RuntimeError(f"complete final relay source lacks {key}")
    if not 1 <= int(source.get("port", 0)) <= 65535:
        raise RuntimeError("complete final relay source port is invalid")
    if any("password" in str(key).lower() for key in source):
        raise RuntimeError("complete final relay may not persist a password")
    fixed = {
        "maximum_consecutive_failures": 3,
        "destination_overwrite_allowed": False,
        "checkpoint_transfer": False,
    ...
    for key, expected in fixed.items():
        if contract.get(key) != expected:
            raise RuntimeError(f"complete final relay changed: {key}")
    if int(contract.get("poll_seconds", 0)) < 30:
        raise RuntimeError("complete final relay polling is too frequent")
    if int(contract.get("timeout_seconds", 0)) < 43200:
        raise RuntimeError("complete final relay timeout is too short")
    if Path(contract["destination"]).resolve() == Path(
        contract["state_path"]
    ).resolve():
        raise RuntimeError("complete final relay destination and state must differ")
...
class CompleteFinalResultRelay:
    def __init__(self, contract_path: Path):
        ...
        if not self.password:
            raise RuntimeError(f"missing complete final relay password: {PASSWORD_ENV}")

    def state(self, status: str, **fields: Any) -> None:
        support.atomic_json(self.state_path, {
            "schema": "final-unsb-route1-complete-final-result-relay-state-v1",
            "updated": support.now(),
            "status": status,
            "pid": os.getpid(),
    ...
    def retrieve(self) -> dict[str, Any]:
        ...
        try:
            with client.open_sftp() as sftp:
                ...
                for name, expected in pointer["final_file_sha256"].items():
                    local = staging / name
                    remote = self._remote("final", name)
                    sftp.get(remote, str(local))
                    if support.file_sha256(local) != expected or _remote_sha256(
                        sftp, remote,
                    ) != expected:
                        raise RuntimeError(f"complete final relay hash mismatch: {name}")
                extras = {
                    EXTRA_FILES[0]: pointer["complete_4090_frontier_sha256"],
                    EXTRA_FILES[1]: pointer["portable_5090_frontier_sha256"],
                }
                for name, expected in extras.items():
                    ...
                    if support.file_sha256(local) != expected or _remote_sha256(
                        ...
                        raise RuntimeError(f"complete final input hash mismatch: {name}")
        finally:
            client.close()
        ...
        for name in names:
            ...
            if target.is_file():
                if target.read_bytes() != source.read_bytes():
                    raise RuntimeError(f"complete final relay destination differs: {name}")
                source.unlink()
            else:
                os.replace(source, target)
        pointer = validate_local_delivery(self.destination)
        support.atomic_json(self.destination / "RELAY_MANIFEST.json", {
            "schema": "final-unsb-route1-complete-final-relay-manifest-v1",
    ...
    def run(self) -> int:
        ...
        while True:
            if time.time() - self.started > int(self.contract["timeout_seconds"]):
                raise TimeoutError("complete final result relay timed out")
            try:
                if not self.pointer_ready():
                    failures = 0
                    self.state("WAITING_FOR_REMOTE4090_COMPLETE_FINAL_POINTER")
                    time.sleep(int(self.contract["poll_seconds"]))
                    continue
                pointer = self.retrieve()
                self.state(
                    "COMPLETE_FINAL_DELIVERY_RETRIEVED",
                    selected_candidate_id=pointer["selected_candidate_id"],
                ...
                failures += 1
                self.state(
                    "TRANSIENT_COMPLETE_FINAL_RELAY_FAILURE",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.init_contract:
        ...
        return 0
    try:
        return CompleteFinalResultRelay(args.contract).run()
    except Exception as error:
        contract = _read_json(args.contract)
        support.atomic_json(Path(contract["state_path"]), {
            "schema": "final-unsb-route1-complete-final-result-relay-fatal-v1",
            "updated": support.now(),
            "status": "FAILED",
            "error": repr(error),
