"""Materialize the one evidence-authorized PC-RSMG mathematical revision."""
...
CANDIDATE_ID = "G2-01-ADAM-METRIC-TANGENTIAL-CONSENSUS"
SPEC = {
    "model": "route1_amtnc",
    "method": {"amtnc_replicates": 2},
    "gate_callable": "run_amtnc_gate",
    "sources": [
        "src/models/sb_model.py",
        "src/models/route1/__init__.py",
        "src/models/route1/amtnc.py",
        "src/models/route1_amtnc_model.py",
...
def _copy_exact(source: Path, destination: Path) -> None:
    ...
    if destination.is_file():
        if destination.read_bytes() != source.read_bytes():
            raise RuntimeError(f"non-identical frozen AM-TNC artifact exists: {destination}")
        return
...
def materialize(output_root: Path) -> dict:
    ...
    request_destination = (
        output_root / "derive" / "revisions" / request_source.name
    )
    _copy_exact(request_source, request_destination)
    authorization = register_candidate_revision(
    ...
    if implementation_path.is_file():
        ...
        if existing != implementation:
            raise RuntimeError("non-identical AM-TNC implementation already exists")
    else:
        write_json(implementation_path, implementation)
    registration = freeze_candidate_derivation(output_root, CANDIDATE_ID)
    return {
        "schema": "final-unsb-route1-causal-revision-materialization-v1",
        "status": "AMTNC_REVISION_FROZEN_FOR_GATES",
