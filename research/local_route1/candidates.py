...
def load_candidate_registration(
    ...
    if require_gate:
        ...
        if implementation.get("model") == "route1_hpcgr":
            evidence = gate.get("evidence", {})
            player_evidence = evidence.get("player_conditional_execution_evidence")
            if not isinstance(player_evidence, dict) or (
                ...
                or player_evidence.get("hnek_active_all_states") is not True
                or player_evidence.get("conditional_expected_field") != "HNEK"
            ):
                raise RuntimeError("HPCGR gate has invalid conditional-field provenance")
            compatibility = evidence.get("component_compatibility_evidence")
            if not isinstance(compatibility, dict) or (
                ...
                or compatibility.get("branch_updates") != [1, 8, 32]
                or compatibility.get("all_parent_state_hashes_preserved") is not True
                or compatibility.get("coordinate_only_exact_hnek_all_rows") is not True
                or compatibility.get(
                    "estimator_only_exact_pcrsmg_proposal_all_rows"
                ) is not True
                or compatibility.get(
                    "observable_only_exact_hnek_excluding_observer_all_rows"
                ) is not True
            ):
